## Hi there 👋

I am Elena, a **Computer Engineering student** at the Faculty of Computer Science & Engineering in Skopje, currently in my final year.  
My fields of interest are **Data Science, Machine Learning, Robotics, and Embedded Systems**.  
I enjoy learning new things, experimenting with projects, and applying my academic knowledge to practical applications.

### 💻 Projects

#### 🚀 Key Projects
- [**Brain Tumor Segmentation**](https://github.com/RebekaManeva/3D-MRI-Segmentation) – Deep learning model (U-Net/V-Net) for semantic segmentation of MRI images using the BraTS2023 dataset.
- [**Pet Image Segmentation**](https://github.com/enikolovska/unet-pet-segmentation) - Semantic segmentation project using a U-Net architecture to isolate pets from backgrounds.
- [**Gardenino Project**](https://github.com/enikolovska/GardeninoProject) – A mobile robotic cart equipped with sensors, designed for planting agricultural crops.  
- [**Plant Diseases Project**](https://github.com/enikolovska/PlantDiseasesProject) – A deep learning model for recognizing plant diseases using Convolutional Neural Networks.
- [**Spatial Interpolation**](https://github.com/enikolovska/SpatialInterpolation) – Spatial interpolation of PM2.5 particle concentrations in the city of Oslo.
- [**BookWormNet**](https://github.com/enikolovska/BookWormNetProject) – A personal web-based e-diary for tracking and managing books I’ve read.  


#### 🛠️ Mini Projects
- [**Diamond Price Prediction**](https://github.com/enikolovska/DiamondPricePrediction) – Predicting diamond prices based on their characteristics using regression models.  
- [**Thoracic Surgery Classification**](https://github.com/enikolovska/ThoracicSurgeryClassification) – Predicting patient mortality after thoracic surgery using machine learning models.
- [**CCMP Simulation**](https://github.com/enikolovska/CCMPSimulation) – Simulation of the WPA2 CCMP protocol using AES-CTR encryption in Java.


### 🌱 Currently learning
- Advanced **Python** for data science  
- Machine learning techniques and model evaluation  
- Data preprocessing and feature engineering  
- Arduino components and embedded systems basics

### 📫 How to reach me
- Email: elenanikolovska03@gmail.com
